...
const CIRCLE_SIZE = SCREEN_WIDTH * 0.4;
...
export const RunnyCircle = ({ programs, onSetSelectProgram } : { programs : Program[], onSetSelectProgram : (program : Program) => void }) => {
  const selectedIcon = useSharedValue<undefined | number>(undefined);
  const [selectedIndex, setSelectedIndex] = useState<number | undefined>(
    undefined
  );

  const [ selectedProgram, setSelectedProgam ] = useState<Program>()
