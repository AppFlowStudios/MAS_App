          ...
          <Pressable className='flex-row items-center'>
            ...
            <View className='flex-col justify-center' style={{width: width / 1.5}}>
              ...
              <View className='flex-row' style={{flexShrink: 1, width: width / 1.5}}>
                {speaker == "MAS" ? <Text className='ml-2 text-gray-500' style={{flexShrink:1}} numberOfLines={1}>{lecture.lecture_speaker} </Text> : <Text className='ml-1 text-gray-500'> {format(lecture.lecture_date, 'PP')}</Text>}
              </View>
            </View>
          </Pressable>
        </Link>
