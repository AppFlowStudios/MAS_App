...
const NotificationCard = ({height , width, index, scrollY, setSelectedNotification, selectedNotification, program_id, programInfo} : NotificationCardProp) => {
  ...
  const onPress = async () => {
    ...
    if(currentSettings){
      ...
      if( settings.includes(CardOptions[index]) ){
        ...
        .eq('program_event_name', programInfo.program_name)
          ...
          if( index == 2 ){
            await Promise.all(program_days.map( async ( days ) => {
              ...
              if( program_days.includes(daysOfWeek[day]) && isBefore( currentDay, programStartTime )){
                ...
                else if( index == 1 && isBefore(currentDay, programStartTime) ){
  ...
  return (
    <Animated.View style={[{ height : height, width : width, borderRadius : 20 }, cardStyle, {marginTop : index === 0 ? 10: 0}, {marginBottom : index === 2 ? 10 : 0}]}>
        <Pressable 
        onPress={handlePress} 
